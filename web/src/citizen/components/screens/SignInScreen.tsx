        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4 z-50 animate-in fade-in">
          <div className="bg-white rounded-3xl p-6 max-w-xs w-full text-center shadow-2xl border border-slate-100">
            ...
            <p className="text-xs text-slate-500 mt-1">
              We sent an emergency SMS OTP to <br />
              <strong className="text-slate-800">{phoneNumber ? `+91 ${phoneNumber}` : '+91 98765 43210'}</strong>
            </p>
            ...
            <div className="flex justify-center gap-2.5 my-4">
              {otp.map((digit, idx) => (
                <input
                  ...
                  className="w-11 h-12 text-center text-lg font-bold border border-slate-200 rounded-xl focus:border-red-500 focus:ring-1 focus:ring-red-500 focus:outline-none"
                />
              ))}
            </div>

            <p className="text-[11px] text-slate-400 mb-4">
              Didn't receive? <button type="button" className="text-red-600 font-semibold underline cursor-pointer">Resend in 24s</button>
            </p>
