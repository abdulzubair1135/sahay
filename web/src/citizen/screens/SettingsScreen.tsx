...
export const SettingsScreen: React.FC<SettingsScreenProps> = ({
  ...
  const [resetEmail, setResetEmail] = useState(userProfile.email || 'sahay1382@gmail.com');
  ...
  const handleRequestResetOtp = async () => {
    ...
    setResetMsg('Sending OTP to ' + (resetEmail || 'sahay1382@gmail.com') + '...');
    try {
      const res = await api.requestPasswordReset(resetEmail || 'sahay1382@gmail.com');
      if (res.success) {
        setResetStep('otp');
        setResetMsg('OTP sent to ' + (resetEmail || 'sahay1382@gmail.com') + '! Check inbox.');
        if (res.data?.devHint) setResetOtp(res.data.devHint);
      } else {
        setResetMsg(res.error?.message || 'Failed to send OTP');
      }
    } catch (e: any) {
      setResetMsg(e.message || 'Error sending OTP');
    } finally {
  ...
  const handleConfirmReset = async () => {
    if (!resetOtp || !newPassword) {
      alert('Please enter both OTP and your new password');
      return;
    }
    setResetLoading(true);
    try {
      const res = await api.changePasswordWithOtp({
        email: resetEmail || 'sahay1382@gmail.com',
        otp: resetOtp,
        newPassword
      });
      if (res.success) {
        alert('Password successfully updated via Email OTP!');
        setResetStep('idle');
        setResetMsg('');
        setResetOtp('');
        setNewPassword('');
  ...
  const handleImageFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (event) => {
      ...
      if (base64) {
        ...
        try {
          const res = await api.uploadBase64Image(base64, 'profiles');
          if (res.success && res.data?.url) {
            onUpdateProfile({ avatarUrl: res.data.url });
          }
        } catch (err) {
  ...
  return (
    <div className="w-full flex-1 flex flex-col justify-between bg-[#F8FAFC] select-none">
      ...
      <main className="flex-1 px-4 py-3 space-y-4 overflow-y-auto pb-24 no-scrollbar text-left">
        ...
        {/* Offline APK Distribution & Share */}
        <section className="space-y-2">
          <h3 className="text-xs font-bold text-slate-900 uppercase tracking-wider px-1">Offline Mesh Distribution</h3>
          <div className="bg-gradient-to-br from-blue-50 to-indigo-50 border border-blue-200/80 rounded-2xl p-4 shadow-xs space-y-3">
            <div className="flex items-center space-x-3">
              ...
              <div className="flex-1">
                <h4 className="text-xs font-bold text-slate-900">Sahay Disaster Mesh Android APK</h4>
                <p className="text-[10px] text-slate-600 mt-0.5">
                  Share directly via Bluetooth, Nearby Share or download without Google Play Store
                </p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-2 pt-1">
              <a
                href="/sahay.apk"
                download="Sahay_Disaster_Mesh.apk"
              ...
              <button
                type="button"
                onClick={() => {
                  navigator.clipboard.writeText(window.location.origin + '/sahay.apk');
                  alert('📋 Direct APK Download Link copied to clipboard!');
                }}
                className="flex items-center justify-center space-x-1.5 py-2.5 px-3 bg-white hover:bg-slate-50 text-blue-800 border border-blue-200 rounded-xl text-xs font-bold transition cursor-pointer"
              >
        ...
        {/* Security & Password Reset via Gmail OTP */}
        <section className="space-y-2">
          <h3 className="text-xs font-bold text-slate-900 uppercase tracking-wider px-1">Account Security &amp; Password</h3>
          <div className="bg-white rounded-2xl p-4 shadow-xs border border-slate-200/80 space-y-3">
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-2">
                ...
                <span className="text-xs font-bold text-slate-800">Password Reset via Gmail OTP</span>
              </div>
            ...
            {resetStep === 'idle' ? (
              <div className="space-y-2">
                <p className="text-[11px] text-slate-500">
                  Request an emergency OTP code dispatched directly to your registered Gmail address:
                </p>
                <div className="flex items-center space-x-2">
                  <input
                    type="email"
                    ...
                    className="flex-1 text-xs border border-slate-200 rounded-xl px-3 py-2 focus:ring-1 focus:ring-red-500 focus:outline-none"
                  />
                  <button
                    type="button"
                    onClick={handleRequestResetOtp}
                    disabled={resetLoading}
                    className="px-3 py-2 bg-red-600 hover:bg-red-700 text-white rounded-xl text-xs font-bold transition-all shadow-xs cursor-pointer"
                  >
                    {resetLoading ? 'Sending...' : 'Send OTP'}
                  </button>
                </div>
                {resetMsg && <p className="text-[11px] text-slate-600 font-medium">{resetMsg}</p>}
              </div>
            ) : (
              <div className="space-y-2.5 pt-1 border-t border-slate-100">
                <p className="text-[11px] text-emerald-600 font-medium">{resetMsg}</p>
                <div>
                  <label className="text-[10px] font-bold text-slate-700 uppercase">6-Digit Email OTP</label>
                  <input
                    type="text"
                    value={resetOtp}
                    onChange={(e) => setResetOtp(e.target.value)}
                    placeholder="Enter 6-digit OTP (e.g. 123456)"
                    className="w-full text-xs border border-slate-200 rounded-xl px-3 py-2 focus:ring-1 focus:ring-red-500 focus:outline-none mt-1"
                  />
                </div>
                <div>
                  <label className="text-[10px] font-bold text-slate-700 uppercase">New Password</label>
                  <input
                    type="password"
                    value={newPassword}
                ...
                <div className="flex items-center space-x-2 pt-1">
                  ...
                  >
                    {resetLoading ? 'Updating...' : 'Confirm Password Update'}
                  </button>
                  <button
                    type="button"
                    onClick={() => setResetStep('idle')}
                    className="px-3 py-2 bg-slate-100 text-slate-700 rounded-xl text-xs font-semibold cursor-pointer"
                  >
                    Cancel
                  </button>
                </div>
