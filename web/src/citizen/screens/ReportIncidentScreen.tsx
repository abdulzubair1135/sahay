...
export const ReportIncidentScreen: React.FC<ReportIncidentScreenProps> = ({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    api.createReport({
      type: incidentType,
      description: details || `Reported ${incidentType} hazard in community area`,
      latitude: 23.0225,
      longitude: 72.5714,
      addressText: locationName,
      images: photos,
      mediaUrl: photos.length > 0 ? photos[0] : '',
      userName: 'Citizen Reporter',
      userPhone: '9876543210'
    }).catch((err) => console.error('Failed to submit report to backend:', err));
  };
